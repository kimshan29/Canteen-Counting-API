...
class Super_model extends CI_model
{
    ...
    public function do_upload_file($data_gambar, $folder_gambar, $type = 'doc|docx|pdf|xls|xlsx|ppt|pptx|gif|jpg|png|swf|txt|flv|mp4|mov|mpeg|mp3|avi|zip|rar', $status = '', $resize = array(),$max_size = '500000')
    {
        ...
        if($this->upload->do_upload($data_gambar)) {
            ...
            /*if(strtoupper($status)== "RESIZE")
            {
                $config = array('image_library' => 'gd2', 'source_image' => $this->upload->data('full_path'), 'new_image' => $this->gallery_path .'/thumbs', 'maintain_ration' => TRUE, 'width' => $width,'height' => $height);
                $this->load->library('image_lib');
                $this->image_lib->clear();
    ...
    public function make_captcha($conf = array())
    {
        ...
        if ( $cap ) {
            ...
            return var_dump($cap);//"Captcha not work" ;
        }
    ...
    function add_log_activity($activity, $crud = NULL)
    {
        $session = sessions();
        ...
        $add['browser']= $this->input->ip_address();
        $add['crud']= $crud;

        $this->db->insert($this->table_log_activity, $add);
    }
    
    public function cronLog($filename, $cronResult = null)
	{
		$param = array(	'datet_time' 	=> date("Y-m-d H:i:s"),
						'file' 		=> (string) $filename,
						'result' 		=> (string) $cronResult,
//						'issuccess' 	=> $isSuccess,
					  );

		$this->db->insert('dc_cron_log', $param);
	}
